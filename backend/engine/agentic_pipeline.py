...
def _build_st_generator_prompt(brand_key: str, system_type: str, program_name: str) -> str:
    ...
    return (
        f"You are a senior industrial PLC engineer. Generate deployable IEC 61131-3 ST code.\n\n"
        f"Target: {brand_key} | {system_type} | PROGRAM {program_name}\n\n"
        f"CRITICAL RULES — every rule enforced. Violation = rejected output:\n"
        f"1. PROGRAM {program_name} ... END_PROGRAM is mandatory.\n"
        f"2. Single VAR...END_VAR. Inputs: I_ prefix. Outputs: Q_ prefix. Internal: M_.\n"
        f"3. Use {vendor['edge_block']} for I_Start, I_Stop, I_Reset edge detection.\n"
        f"4. CASE M_State OF — use integer state IDs (0=Idle, 1..N active, last=Fault).\n"
        f"5. EVERY CASE body MUST have REAL logic — NOT placeholder comments:\n"
        f"   Idle (0):  IF StartTrig.Q THEN M_State := 1; END_IF;\n"
        f"   Active (1..N): Check the sensor/timer that causes the transition.\n"
        f"                  Do NOT leave the body empty. Do NOT set all outputs FALSE.\n"
        f"   Fault (last): Q_Alarm := TRUE; IF ResetTrig.Q AND I_EStop THEN M_State := 0; END_IF;\n"
        f"6. E-Stop: IF NOT I_EStop THEN M_State := <fault_id>; END_IF; — BEFORE CASE.\n"
        f"7. Timers: declare {vendor['timer_block']}, call OUTSIDE CASE (every scan), use .Q in CASE.\n"
        f"8. Outputs: Q_xxx := (M_State = N) OR (M_State = M); — at BOTTOM, ONCE each.\n"
        f"9. Declare I_Reset : BOOL in VAR if fault reset is needed.\n"
        f"10. Return ONLY valid ST code. No markdown. No explanation. No stub comments.\n"
    )
